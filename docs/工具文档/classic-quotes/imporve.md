...
export class ClassicQuotesService {
  ...
  normalize(records: any[], fields: any[]): Quote[] {
    return [];
  }
  buildPayload(quotes: Quote[]): Payload {
...
if (!forceRefresh && cacheExists && Date.now() - payload.updatedAt < ttlMs) {
  return new Response(JSON.stringify(payload), {
    status: 200,
    headers: { ETag, "Last-Modified": lm, "Cache-Control": cacheHdr },
  });
}
```
...
- 我将按上述 Phase 顺序实施代码重构与优化，并提供测试与文档更新，确保平滑上线。
